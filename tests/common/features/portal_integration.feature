...
Feature: XDG desktop portal integration
  ...
  Scenario: Document portal broker is running
    * Run SSH command: "systemctl --user is-active xdg-document-portal 2>/dev/null && echo active || systemctl --user status xdg-document-portal 2>/dev/null | grep -cE 'active|running'"
