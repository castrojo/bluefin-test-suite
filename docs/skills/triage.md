---
name: testsuite-triage
description: >-
  How to triage issues and PRs in projectbluefin/testsuite. Use when reviewing
  the backlog, classifying stale issues, applying status labels, or deciding
  whether an issue is ready to claim versus blocked on another repo.
metadata:
...
`projectbluefin/testsuite` uses a small set of lifecycle labels to keep the
...
- The work belongs in `projectbluefin/testing-lab` or another repo (file/link there)
...
4. **Close completed/duplicate issues.** Use evidence from code or comments.
   ...
   ```bash
   GH_REPO=projectbluefin/testsuite gh issue edit <number> --remove-label "status/queued"
   ```
...
| Harness/CI issue | `structural` + `status/queued` | May need a split PR to `projectbluefin/testing-lab` |
