...
| **Security** | Secrets in CI, cosign/signing changes, any `<readonly-upstream>/*` interaction, COPR sources in runner |
...
**NEVER** create issues, PRs, comments, forks, or any other programmatic write to any `<readonly-upstream>/*` repository. Read-only `gh api` calls are permitted. If a task requires writing to `ublue-os`, stop and tell the human to report it manually.
